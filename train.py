...
for epoch in range(continue_train, total_epoch):
    ...
    for x, y in train_dataset:
        ...
        if show_batch_loss:
            print(
                "Epoch {:03d}, Batch {:03d}: Train Loss: {:.3f}".format(
                    epoch, batch_index, loss_value
                )
            )
            batch_index += 1
    ...
    print(
        "Epoch {:03d}: Train Loss: {:.3f}, Accuracy: {:.5%}".format(
    ...
    if not ((epoch + 1) % 5):
        ...
        for x, y in test_dataset:
            val_accuracy(y, model(x))
        print(
            "Epoch {:03d}, Validation accuracy: {:.5%}".format(
...
model.save(os.path.join(model_path, "blaze_pose_pretrained"))
model.summary()
